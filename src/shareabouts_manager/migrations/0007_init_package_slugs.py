...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        def uniquify(text, model, field_name):
            count = 1
            original_text = text
            while model.objects.filter(**{field_name: text}).exists():
                count += 1
                text = '%s-%s' % (original_text, count)
            return text

        for package in orm.AccountPackage.objects.all():
            package.slug = uniquify(slugify(package.name), orm.AccountPackage, 'slug')
            package.save()
